...
PERSONAS = {
    ...
    "bear":  {"name": "Bear 空头", "voice": "寻找盈利放缓、估值约束、趋势走弱和资本行为中的下行风险"},
    "macro": {"name": "Macro 宏观", "voice": "结合行业指标、利率货币和相对指数表现判断外部环境"},
    "risk":  {"name": "Risk 风控", "voice": "量化波动、回撤、现金流、资金和重大事件风险"},
...
ROLE_FOCUS = {
    ...
    "macro": (
        ...
        "重点说明外部环境对公司判断提供了什么支持、约束或不确定性。"
    ),
    "risk": (
        "给出风险水平和触发条件，优先量化波动、回撤、现金流、资金与事件压力；"
        "股东户数变化只能说明"
        "集中度变化，不能推断投资者身份、持仓信心或未来抛售压力。"
    ),
}

_RESEARCH_PROFILE_INTERPRETATION = (
    "这些材料是面向个股判断的结构化研究画像，不是工具运行日志。"
    "优先解释公司经营、财务、估值、历史市场表现、同行比较、资金、股东行为、重大事件与宏观环境。"
    "财务与盈利画像优先使用最近披露的季度财务报表，快报只作补充；"
    "同比是最新季度与上年同季度累计口径比较，估算 ROE 已在 assumptions 标明年化方法；"
    "不得自行推断营收或利润变化由投资收益、赔付、准备金、产品结构或其他未提供原因造成；"
    "不得自行判断增长是否来自小基数，除非材料提供上年同期绝对值和历史序列；"
    "金融企业若未提供 cash_to_profit_ratio，就不得用经营现金流/利润倍数评价利润质量；"
    "估值画像的 PE/PB 是快照估算，缺少同行或历史分位时不能单独判断贵或便宜；"
    "市场画像只描述历史收益、相对强弱、波动和回撤，不是未来涨跌预测；"
    "行业分位只代表当前同行样本位置；"
    "资金画像中的融资、北向、龙虎榜和大宗交易属于不同群体，不得合并虚构净流入；"
    "不得由这些指标推断大资金动机、恐慌、换手性质、存量博弈或资金合力；"
    "股东户数变化不能识别投资者身份、耐心、承接盘或筹码锁定性，也不能直接推出未来卖压；"
    "产量与装车量若一个是当月同比、另一个是累计同比，只能并列描述，不能直接推出需求、库存、储能去向或供需缺口；"
    "管理层交流、公告和业绩预告不等于结果已经实现；"
    "回购和分红记录不能直接写成内部信心、股价支撑或安全垫；"
    "宏观画像只提供环境背景，没有传导证据时不能直接推出个股方向。"
)
...
class MockLLM:
    ...
    def argue(self, *, side: str, symbol: str, evidence: list[dict[str, Any]]) -> str:
        ...
        joined = "；".join(bits) if bits else "没有可解释的研究画像"
        ...
        if side == "bear":
            return f"{symbol} 的主要弱点和下行情景是：{joined}。盈利、估值和趋势能否改善是判断重点。"
        if side == "macro":
            return f"从行业与市场环境看 {symbol}：{joined}。这里判断的是外部环境和相对强弱，不替代公司基本面评价。"
        if side == "risk":
            return f"{symbol} 当前需要重点管理的风险是：{joined}。风险评价关注波动、回撤、现金流、资金和事件，不等同于公司好坏。"
    ...
    def rebut(
        ...
        *,
        ...
        return (
            ...
            f"目前审计状态为 {statuses}；我的依据是 {summaries or '现有研究画像'}。"
...
class OpenAICompatLLM:
    ...
    def _chat(self, system: str, user: str, *, want_json: bool = False) -> str:
        ...
        try:
            ...
            return "（模型说明暂不可用；请直接查看下方研究数据和风险提示。）"
    ...
    def _chat_stream(self, system: str, user: str) -> Iterator[str]:
        ...
        try:
            ...
            if emitted:
                ...
                yield "（模型说明暂不可用；请直接查看下方研究数据和风险提示。）"
    ...
    def _argue_prompt(
        *,
        ...
        system = (
            ...
            f"{ROLE_FOCUS[side]}"
            "只解释给定的股票研究画像。不得补写未提供的指标、来源或调用。"
            "严禁给出买入/卖出/目标价/收益承诺；你是在帮小白理解，不是荐股。"
            f"{_RESEARCH_PROFILE_INTERPRETATION}"
    ...
    def _rebut_prompt(
        *,
        ...
        system = (
            ...
            "只能使用给定的股票研究画像，不得补写数据、指标、来源或工具调用。"
            "严禁给出买卖建议、目标价或收益承诺。"
            f"{_RESEARCH_PROFILE_INTERPRETATION}"
